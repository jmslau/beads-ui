 ...
 * @property {boolean} [sortable] - Header is clickable to sort by `key`.
 */
...
export function createColumnResizer(options) {
  ...
  return {
    ...
    /**
     * Render the header cells, each with a resize handle. Columns marked
     * `sortable` render a clickable sort button (with `aria-sort` and a ▲/▼
     * indicator) when `sort` is provided.
     *
     * @param {{ state: SortState, onSort: (key: string) => void }} [sort]
     */
    headerCells(sort) {
      return html`${columns.map((column, index) => {
          const sort_ctx = column.sortable ? sort : undefined;
          const active = Boolean(sort_ctx && sort_ctx.state.key === column.key);
          const aria_sort = sort_ctx
            ? active
              ? sort_ctx.state.dir === 'asc'
                ? 'ascending'
                : 'descending'
              : 'none'
            : nothing;
          ...
          >
            ${sort_ctx
              ? html`<button
                  ...
                  @click=${() => sort_ctx.onSort(column.key)}
                >
                  <span class="th-sort__label text-truncate"
                    >${column.label}</span
                  >
              ...
              : html`<span class="text-truncate">${column.label}</span>`}
            <span
              class="col-resizer"
              role="separator"
